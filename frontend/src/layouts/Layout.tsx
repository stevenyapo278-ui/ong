    ...
    <ThemeProvider>
      <div className="min-h-screen flex flex-col bg-background text-foreground transition-colors duration-300">
        ...
        <footer className="bg-slate-900 border-t border-slate-800 pt-24 pb-12 overflow-hidden relative">
          ...
          <div className="absolute -top-24 -right-24 w-96 h-96 bg-indigo-600/5 rounded-full blur-[100px]"></div>
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 sm:gap-16 mb-20 relative z-10">
            ...
            <div className="space-y-8 text-center sm:text-left">
              <div className="flex items-center justify-center sm:justify-start group">
                <div className="flex items-center justify-center w-12 h-12 bg-indigo-600 rounded-[18px] text-white shadow-lg shadow-indigo-500/20 group-hover:scale-110 transition-transform">
                  <Heart size={24} className="fill-white" />
                </div>
                <div className="ml-4 text-left">
                  <p className="text-2xl font-black text-white tracking-tight">ONG</p>
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest -mt-1">IMPACT</p>
                </div>
              </div>
            ...
            <div className="space-y-8 text-center sm:text-left">
              ...
              <ul className="space-y-4">
                {['Accueil', 'Actualités', 'Nos Missions', 'Don'].map((item, i) => (
                  <li key={i}>
                    <button className="text-slate-400 font-bold hover:text-indigo-400 transition-colors text-sm hover:translate-x-1 inline-block transform transition-transform">{item}</button>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="space-y-8 text-center sm:text-left">
              ...
              <ul className="space-y-6">
                <li className="flex flex-col sm:flex-row items-center sm:items-start gap-3 text-slate-400 font-bold text-sm">
                  ...
                  <span>123 Avenue de l'Impact, 75001 Paris, France</span>
                </li>
                <li className="flex flex-col sm:flex-row items-center sm:items-start gap-3 text-slate-400 font-bold text-sm">
                  <Mail size={18} className="text-indigo-500 shrink-0" />
                  <span>contact@ong-impact.org</span>
                </li>
                <li className="flex flex-col sm:flex-row items-center sm:items-start gap-3 text-slate-400 font-bold text-sm">
          ...
          <div className="max-w-7xl mx-auto px-6 border-t border-slate-800 pt-12 pb-8 text-center flex flex-col items-center gap-8 relative z-10">
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">
              &copy; {new Date().getFullYear()} ONG IMPACT. PROPULSÉ PAR LA TECHNOLOGIE & L'HUMANITÉ.
            </p>
